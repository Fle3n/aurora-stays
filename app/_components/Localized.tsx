...
export function DetailPage({ lang, slug }: { lang: Lang; slug: string }) {
  ...
  return <main className="inner-page"><header><a href="/">{c.brand}</a><nav>{Object.entries(c.pages).map(([s,v])=><a key={s} href={`/${lang}/${s}`}>{v.label}</a>)}<a href={c.switchTo}>{c.switchLabel}</a></nav></header><section className="inner-hero"><p>{p.eyebrow}</p><h1>{p.title}</h1><span>{p.intro}</span></section><section className="inner-cards">{p.cards.map(([t,d],i)=><article key={t}><small>0{i+1}</small><h2>{t}</h2><p>{d}</p><a href="mailto:stay@aurora.example">{lang==="ru"?"Узнать больше":"Discover"} ↗</a></article>)}</section><footer><p>{p.note}</p><a href="/">{c.back} ↑</a></footer></main>;
}
