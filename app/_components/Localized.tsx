...
export function DetailPage({ lang, slug }: { lang: Lang; slug: string }) {
  const c = content[lang]; const p = c.pages[slug as keyof typeof c.pages] || c.pages.stays;
  return <main className="inner-page" data-lang={lang}><header><a href={lang==="ru"?"/ru":"/"}>{c.brand}</a><nav>{Object.entries(c.pages).map(([s,v])=><a key={s} href={`/${lang}/${s}`}>{v.label}</a>)}<a className="page-lang" href={`/${lang==="ru"?"en":"ru"}/${slug}`}>{c.switchLabel}</a></nav></header><section className="inner-hero"><p>{p.eyebrow}</p><h1>{p.title}</h1><span>{p.intro}</span></section><section className="inner-cards">{p.cards.map(([t,d],i)=><article key={t}><small>0{i+1}</small><h2>{t}</h2><p>{d}</p><a href="mailto:stay@aurora.example">{lang==="ru"?"Узнать больше":"Discover"} ↗</a></article>)}</section><footer><p>{p.note}</p><a href={lang==="ru"?"/ru":"/"}>{c.back} ↑</a></footer></main>;
}
